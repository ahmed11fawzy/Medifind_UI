...
export const RequestsReview = () => {
  ...
  // Fetch requests and orders on component mount
  useEffect(() => {
    fetchData(req_url).then((data) => {
      if (data) setRequests(data);
    });
  ...
  const handleAccept = async (url, id, setFunction, medicineName) => {
    try {
      ...
      // Decrement the medicine quantity in the backend
      const decrementResponse = await fetch(
        `http://localhost:7777/medicine/decrement/${encodeURIComponent(medicineName)}`,
        { method: "PATCH" }
      );

      if (!decrementResponse.ok) {
        throw new Error(`Failed to decrement quantity: ${decrementResponse.status}`);
      }
  ...
  const handleReject = async (url, id, setFunction) => {
    try {
      const response = await fetch(`${url}/${id}`, {
        method: "PATCH",
        headers: {
                ...
                <div className="col-6 mt-5 ms-2">
                  <div>
                    <div className="mb-0 mt-5">
                      ...
                      <AddBtn
                        onClick={() =>
                          handleAccept(req_url, request._id, setRequests, request.req_name)
                        }
                        style={{
                          width: "30%",
                          marginTop: "10px",
                ...
                <div className="col-6 mt-5 ms-2">
                  <div>
                    <div className="mb-0 mt-5">
                      ...
                      <AddBtn
                        onClick={() =>
                          handleAccept(order_url, order._id, setOrders, order.req_name)
                        }
                        style={{
                          width: "30%",
                          marginTop: "10px",
